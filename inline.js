...
if(typeof GRAMMAR_DATA==='undefined'){var GRAMMAR_DATA=[]}
if(typeof QUIZ_DATA_HIGH==='undefined'){var QUIZ_DATA_HIGH=[]}
if(typeof QUIZ_DATA_NORMAL==='undefined'){var QUIZ_DATA_NORMAL=[]}
if(typeof QUIZ_DATA_REAL==='undefined'){var QUIZ_DATA_REAL=[]}
var levelMap={zk:0,gk:1,cet4:2,cet6:3,ky:4,ielts:5,toefl:6,gre:7};
var levelLabel={zk:'中考',gk:'高考',cet4:'四级',cet6:'六级',ky:'考研',ielts:'雅思',toefl:'托福',gre:'GRE'};
// SM-2
...
function lp(){try{return JSON.parse(localStorage.getItem('en')||'{}')}catch(e){return{}}}
function sp(p){localStorage.setItem('en',JSON.stringify(p))}
...
function st(n){if(n>=5)return '★★★★★';if(n>=4)return '★★★★';if(n>=3)return '★★★';if(n>=2)return '★★';if(n>=1)return '★';return '☆'}
function fc(n){if(n>=5)return 'fb-s';if(n>=4)return 'fb-a';if(n>=3)return 'fb-b';if(n>=1)return 'fb-c';return ''}
// 统一拖拽绑定/解绑，避免全局 onmousemove 互相覆盖
var _dragMove=null,_dragEnd=null;
function bindDrag(onMove,onEnd){_dragMove=onMove;_dragEnd=onEnd;document.addEventListener('mousemove',onMove);document.addEventListener('mouseup',onEnd);document.addEventListener('touchmove',onEnd,{passive:false});document.addEventListener('touchend',onEnd)}
...
function startStudy(f){document.getElementById('sp').style.display='block';document.getElementById('lm').style.display='none';curF=f;const data=f==='all'?VOCAB:VOCAB.filter(v=>v.levels&&v.levels.includes(f));q=[...data].sort((a,b)=>b.stars-a.stars);ci=resumeSession(f);if(ci>0)showT('📖 继续上次的进度 ('+(ci+1)+'/'+q.length+')');showC()}
function showC(){if(ci>=q.length){showComp();return}const v=q[ci];fl=false;document.getElementById('c3d').classList.remove('f');document.getElementById('cft').innerHTML='<span class="fb '+fc(v.stars)+'">'+st(v.stars)+' <span style="font-size:10px;color:#888">'+v.stars+'回</span></span>';document.getElementById('cfw').textContent=v.word;document.getElementById('cfr').textContent=v.phonetic;document.getElementById('cft2').textContent=v.type||'';document.getElementById('cbm').textContent=v.meaning;document.getElementById('cbt').textContent=v.type||'';document.getElementById('cbej').textContent=v.definition||'';document.getElementById('cber').textContent='';document.getElementById('cbec').textContent=''||'';document.getElementById('cbb').innerHTML='<span class="fb '+fc(v.stars)+'">'+st(v.stars)+'</span>';document.getElementById('abtns').style.display='none';document.getElementById('pt').textContent=(ci+1)+'/'+q.length;document.getElementById('pf').style.width=Math.max(3,Math.round(ci/q.length*100))+'%'}
...
function rate(ql){const v=q[ci];const p=lp();const cur=p[v.id]||{};const r=sm2(cur,ql);p[v.id]={...r,word:v.word};sp(p);if(ql===0)toggleMark(v.id,"red");else if(ql===3)toggleMark(v.id,"yellow");else if(ql===4)toggleMark(v.id,"green");ci++;if(ci<q.length)setTimeout(showC,200);else showComp()}
function showComp(){document.getElementById('sp').style.display='block';document.getElementById('csc').style.display='none';document.getElementById('abtns').style.display='none';const p=lp();let h=0,e=0;q.forEach(v=>{const c=p[v.id];if(c&&c.repetitions>=2)e++;else h++});document.getElementById('rn1').textContent=q.length;document.getElementById('rn2').textContent=h;document.getElementById('rn3').textContent=e;document.getElementById('comp').classList.add('show');clearSession();if(typeof upH==='function')upH();window._sd=false;unbindDrag()}
function exitS(){saveSession();document.getElementById('sp').style.display='none';document.getElementById('comp').classList.remove('show');document.getElementById('csc').style.display='block';document.getElementById('lm').style.display='block';window._sd=false;unbindDrag()}
let dwId=0;function openD(v){dwId=v.id;document.getElementById('drw').textContent=v.word;document.getElementById('drr').textContent=v.phonetic;document.getElementById('drm').textContent=v.meaning;document.getElementById('drt').textContent=v.type||'';const fbc=fc(v.stars);document.getElementById('drfb').innerHTML='<span class="fb '+fbc+'">'+st(v.stars)+' <span style="font-size:10px">'+v.stars+'回</span></span>';document.getElementById('dre1j').textContent=v.definition||'';document.getElementById('dre1r').textContent='';document.getElementById('dre1c').textContent=''||'';document.getElementById('dw').classList.add('open');document.getElementById('ol').classList.add('open');updateBookBtn(dwId)}
...
let curF='cet6',curSort='freq',curG='all',curGSort=null,q=[],ci=0,fl=false,kanjiMode=0,_vocabOrder=null,catMode='type',catSelected=null,curStudyCount='all';
...
function queueSpeak(text,opts){
  ...
  opts=opts||{};
  _speechQueue.push({text:String(text),lang:opts.lang||'en-US',rate:opts.rate||0.9,voice:opts.voice||null});
...
function speakW() { if (ci < q.length) speak(q[ci].phonetic || q[ci].word); }
function speakE() { if (ci < q.length) speak(q[ci].definition || q[ci].word); }
function speakDW() {
    if (!dwId) return;
...
function speakGP(id) {
    ...
    if (gr) speak(gr.definition || gr.pattern);
}
...
// ── 生词本──────────
const BOOK_KEY = 'en_book';

// ── 新增：颜色标记 ──────────
const MARK_KEY = 'en_mark';
function getMarks(){
...
// ── 搜索收藏（独立于生词本）──
const SEARCH_BOOK_KEY = 'en_search_book';
...
function toggleSearchBook(item){
  ...
  else { list.unshift({id:item.id, word:item.word, reading:item.phonetic||'', meaning:item.meaning||'', level:item.level||'', time:Date.now()}); showT('已加入搜索收藏 ✅'); }
  saveSearchBook(list);
    ...
    if (curTab === 'search') {
      ...
      sb.forEach(function(item){
        ...
        d.innerHTML = '<div class="vt"><span class="vl '+(item.level==='n3'?'l3':item.level==='n2'?'l2':'l1')+'">'+(item.level||'N3').toUpperCase()+'</span></div>'
          ...
          + '<div class="vr">'+escHtml(item.phonetic)+'</div>'
      ...
      book.forEach(function(item){
        ...
        if (item.type === 'ai') {
          ...
          d.innerHTML = '<div class="vt"><span class="vl '+(item.level==='n3'?'l3':item.level==='n2'?'l2':'l1')+'">'+(item.level||'AI').toUpperCase()+'</span></div>'
            + '<div class="vw">'+escHtml(item.word)+'</div>'
            + '<div class="vr">'+escHtml(item.phonetic||'')+'</div>'
            + '<div class="vm">'+escHtml(item.meaning||'')+' <span style="font-size:10px;color:#a855f7;margin-left:4px">AI搜</span></div>';
          d.onclick = function(){ showT('🔍 "'+item.word+'" — '+item.meaning); };
        } else {
          // 词库词
          var v = VOCAB.find(function(x){ return x.id === item.id; });
          if (!v) return;
          d.innerHTML = '<div class="vt"><span class="vl '+(v.level==='cet4'?'l3':'l2')+'">'+(levelLabel[v.level?v.level.split('/')[0]:'cet6']||'CET6')+'</span></div>'
            + '<div class="vw">'+v.word+'</div><div class="vr">'+v.phonetic+'</div><div class="vm">'+v.meaning+'</div>';
...
function studyDragStart(e){window._sd=true;studySeek(e);bindDrag(function(e2){if(window._sd)studySeek(e2)},function(){window._sd=false;unbindDrag()})}

// === 自动播放（三套独立系统）===
// 系统 #1 ap* — 单词墙的自动播放（从词汇页启动）
// 系统 #2 vAp* — 单词学习的自动播放（从学习页/复习页启动）
// 系统 #3 gAp* — 语法页的自动播放
// 三套逻辑结构相似但数据源不同，维护时注意三处同步修改
...
function apShowCard(){if(_apStop||_apIdx>=_apQueue.length){apDone();return}if(_apPaused){_apTimer=setTimeout(apShowCard,200);return}var v=_apQueue[_apIdx];var el=document.getElementById('apScreen');el.querySelector('.ap-word').textContent=v.word;el.querySelector('.ap-read').textContent=v.phonetic;el.querySelector('.ap-mean').textContent=v.meaning;var ex=el.querySelector('.ap-exjp');var exc=el.querySelector('.ap-excn');if(_apBoth&&v.definition){ex.textContent=v.definition;ex.style.display='block';exc.textContent=''||'';exc.style.display=''?'block':'none'}else{ex.style.display='none';exc.style.display='none'}var pct=Math.round((_apIdx+1)/_apQueue.length*100);el.querySelector('.ap-progress-bar').style.width=pct+'%';el.querySelector('.ap-counter').textContent=(_apIdx+1)+'/'+_apQueue.length;apSyncMarks(v);clearSpeechQueue();queueSpeak(v.word);if(_apBoth&&v.definition)queueSpeak(v.definition);var totalSpeechTime=_apBoth&&v.definition?5000:2500;_apIdx++;clearTimeout(_apTimer);_apTimer=setTimeout(apShowCard,totalSpeechTime+_apInterval)}
...
function upP(){
  ...
  if(session&&cont){
    ...
    if(info){
      var lv={gk:'高中',cet4:'四级',cet6:'六级',ky:'考研',ielts:'雅思',toefl:'托福',gre:'GRE'}[session.filter]||session.filter;
...
function planCalcEstimate(){
  ...
  var total=0;var parts=[];lvls.forEach(function(b){var lv=b.getAttribute('data-lvl');var cnt=0;VOCAB.forEach(function(v){if(v.levels&&v.levels.includes(lv))cnt++});total+=cnt;parts.push(lv.toUpperCase()+'('+cnt+')')});
...
function gApShowCard(){if(_gApStop||_gApIdx>=_gApQueue.length){gApDone();return}if(_gApPaused){_gApTimer=setTimeout(gApShowCard,200);return}var gr=_gApQueue[_gApIdx];var el=document.getElementById('gApScreen');el.querySelector('.ap-word').textContent=gr.pattern;el.querySelector('.ap-read').textContent=''||'';el.querySelector('.ap-mean').textContent=gr.meaning;var ex=el.querySelector('.ap-exjp');var exc=el.querySelector('.ap-excn');if(_gApBoth&&gr.definition){ex.textContent=gr.definition;ex.style.display='block';exc.textContent='';exc.style.display=gr.ex_cn?'block':'none'}else{ex.style.display='none';exc.style.display='none'}var pct=Math.round((_gApIdx+1)/_gApQueue.length*100);el.querySelector('.ap-progress-bar').style.width=pct+'%';el.querySelector('.ap-counter').textContent=(_gApIdx+1)+'/'+_gApQueue.length;gApSyncMarks(gr);clearSpeechQueue();queueSpeak(gr.definition||gr.pattern);if(_gApBoth&&gr.definition)queueSpeak(gr.definition);var totalSpeechTime=_gApBoth&&gr.definition?5000:2500;_gApIdx++;clearTimeout(_gApTimer);_gApTimer=setTimeout(gApShowCard,totalSpeechTime+_gApInterval)}
...
function gApStop(){_gApStop=true;clearTimeout(_gApTimer);_gApTimer=null;unbindDrag();document.getElementById('gApScreen').classList.remove('show')}
function gApDone(){clearTimeout(_gApTimer);_gApTimer=null;unbindDrag();document.getElementById('gApScreen').classList.remove('show');showT('🎉 语法自动播放完成！共 '+_gApQueue.length+' 条');go('grammar')}
...
function renderAutoPlayOptions(){
  ...
  c.innerHTML='<div style="padding:20px;max-width:600px;margin:0 auto">'
    ...
    +'</div>'
    // 考级分类（带勾选）
    +'<div class="ap-section" style="padding:14px 16px;background:rgba(255,255,255,0.03);border-radius:10px;margin-bottom:12px;border:1px solid rgba(255,255,255,0.06)">'
    ...
    +lvls.map(function(l){var a=selLvls.indexOf(l)>=0;return '<span class="ap-tag'+(a?' ap-tag-on':'')+'" data-lvl="'+l+'" onclick="_apToggleLvl(this,\''+l+'\')" style="display:inline-block;padding:6px 16px;border-radius:14px;font-size:13px;font-weight:600;cursor:pointer;transition:all 0.2s;'+(a?'background:linear-gradient(135deg,#e94560,#ff6b9d);color:#fff':'background:rgba(255,255,255,0.06);color:#64748b')+'">'+lvlNames[l]+'</span>'}).join('')
    +'</div>'
    +'</div>'
    // 场景分类（带勾选）
    +'<div class="ap-section" style="padding:14px 16px;background:rgba(255,255,255,0.03);border-radius:10px;margin-bottom:12px;border:1px solid rgba(255,255,255,0.06)">'
    ...
    +ALL_CATEGORIES.map(function(cat){var a=selCats.indexOf(cat)>=0;return '<span class="ap-tag'+(a?' ap-tag-on':'')+'" data-cat="'+cat.replace(/'/g,'\\\'')+'" onclick="_apToggleCat(this)" style="display:inline-block;padding:5px 12px;border-radius:14px;font-size:11px;cursor:pointer;transition:all 0.2s;'+(a?'background:linear-gradient(135deg,#a855f7,#7c3aed);color:#fff':'background:rgba(255,255,255,0.06);color:#64748b')+'">'+cat+'</span>'}).join('')
    +'</div>'
    +'</div>'
    // 形式
    +'<div style="padding:14px 16px;background:rgba(255,255,255,0.03);border-radius:10px;margin-bottom:12px;border:1px solid rgba(255,255,255,0.06)">'
    ...
    +[['word','单词'],['word_sent','单词+例句']].map(function(f){
      ...
      return '<span class="ap-tag'+(cur===val?' ap-tag-on':'')+'" data-format="'+val+'" onclick="_apSetFormat(this,\''+val+'\')" style="display:inline-block;padding:6px 16px;border-radius:14px;font-size:13px;cursor:pointer;transition:all 0.2s;'+(cur===val?'background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff':'background:rgba(255,255,255,0.06);color:#64748b')+'">'+label+'</span>'
    }).join('')
    +'</div>'
    +'</div>'
    // 播放间隔
    ...
    +[[0,'0秒'],[0.5,'0.5秒'],[1,'1秒'],[2,'2秒'],[3,'3秒']].map(function(s){
      ...
      return '<span class="ap-tag'+(speed===val?' ap-tag-on':'')+'" data-speed="'+val+'" onclick="_apSetSpeed(this,'+val+')" style="display:inline-block;padding:6px 16px;border-radius:14px;font-size:13px;cursor:pointer;transition:all 0.2s;'+(speed===val?'background:linear-gradient(135deg,#4ecca3,#2ecc71);color:#fff':'background:rgba(255,255,255,0.06);color:#64748b')+'">'+label+'</span>'
    }).join('')
    +'</div>'
    +'</div>'
    // 数量
    +'<div style="padding:14px 16px;background:rgba(255,255,255,0.03);border-radius:10px;margin-bottom:20px;border:1px solid rgba(255,255,255,0.06)">'
    +'<div style="font-size:14px;font-weight:600;color:#e2e8f0;margin-bottom:10px">🔢 单词数量</div>'
    +'<div style="display:flex;gap:8px;flex-wrap:wrap">'
    +[10,20,50,100].map(function(n){return '<span class="ap-tag'+(count===n?' ap-tag-on':'')+'" data-count="'+n+'" onclick="_apSetCount(this,'+n+')" style="display:inline-block;padding:6px 16px;border-radius:14px;font-size:13px;cursor:pointer;transition:all 0.2s;'+(count===n?'background:linear-gradient(135deg,#f5a623,#ffd700);color:#000':'background:rgba(255,255,255,0.06);color:#64748b')+'">'+n+'个</span>'}).join('')
    +'<span class="ap-tag'+(count===0?' ap-tag-on':'')+'" data-count="0" onclick="_apSetCount(this,0)" style="display:inline-block;padding:6px 16px;border-radius:14px;font-size:13px;cursor:pointer;transition:all 0.2s;'+(count===0?'background:linear-gradient(135deg,#f5a623,#ffd700);color:#000':'background:rgba(255,255,255,0.06);color:#64748b')+'">全部</span>'
    +'</div>'
    +'</div>'
    // 开始按钮
    +'<button onclick="startVocabAutoPlay()" style="width:100%;padding:16px;border-radius:14px;border:none;background:linear-gradient(135deg,#e94560,#ff6b9d);color:#fff;font-size:18px;font-weight:700;cursor:pointer;box-shadow:0 4px 24px rgba(233,69,96,0.3);transition:all 0.2s">🚀 开始播放</button>'
    +'</div>';
}
...
function vApShowCard(){
  ...
  document.querySelector('#gApScreen .ap-read').textContent=w.phonetic;
  document.querySelector('#gApScreen .ap-mean').textContent=w.meaning;
  document.querySelector('#gApScreen .ap-exjp').textContent=showSent?(w.definition||''):'';
  document.querySelector('#gApScreen .ap-excn').textContent=showSent?(''):'';
...
function vApSpeak(w,showSent){
  try{
    clearSpeechQueue();
    queueSpeak(w.word);
    if(showSent&&w.definition)queueSpeak(w.definition,{rate:0.7});
  }catch(e){}
}
...
function doAISearch(q,localResults){
  ...
  var isSentence = q.trim().split(/\s+/).length > 4 || q.length > 30;
  // 句子用更多 token
  ...
  var prompt;
  if (isChinese) {
    prompt='Translate to natural English. Format:\n'+
      'English:\n'+
      'Pronunciation:\n'+
      'Explanation:\n\n'+
      'Rules:\n'+
      '- English: most natural/common English translation\n'+
      '- Pronunciation: phonetic transcription\n'+
      '- Explanation: Chinese explanation of usage\n\n'+
      'Chinese: '+q;
  } else {
    prompt='Analyze the English word/sentence. Format (one per line, write N/A if none):\n'+
      '中文翻译：\n'+
      '外来语原词：\n'+
      '日文汉字：\n'+
      (isSentence ? '单词解析：\n每个单词的【读音】+【中文释义】\n'+
  ...
  callAI(_searchConfig.apiUrl,_searchConfig.model,[{role:'user',content:prompt}],maxTk,_searchConfig.apiKey).then(function(txt){
    ...
    if (isChinese) {
      ...
      txt.split('\n').forEach(function(line){
        var m=line.match(/^English[：:]?\s*(.*)/i);if(m)cn=m[1];
        m=line.match(/^Explanation[：:]?\s*(.*)/i);if(m)kanji=m[1];
        m=line.match(/^Notes?[：:]?\s*(.*)/i);if(m)note=m[1];
      });
      // 容错
...
function searchLocal(q){
  ...
  for(var i=0;i<VOCAB.length;i++){
    ...
    if(v.word.indexOf(ql)>=0||(v.phonetic&&v.phonetic.indexOf(ql)>=0)||(v.meaning&&v.meaning.indexOf(ql)>=0)){
...
function calcScore(v,ql){
  ...
  else if(v.phonetic&&v.phonetic.indexOf(ql)>=0)s=60;
  ...
  for(var i=0;i<results.length;i++){
    ...
    if(r.type==='vocab'){
      ...
      html+='<div class="search-result-item" style="cursor:default">'+
        ...
        '<span class="search-result-reading">'+escHtml(v.phonetic||'')+'</span>'+
      ...
      html+='<div class="search-result-item" onclick="goToGrammar('+(g.id||0)+')">'+
        ...
        '<span class="search-result-level sl-'+(g.level||'n3')+'">'+((levelLabel[g.level]||g.level.toUpperCase()))+'</span>'+
        '</div>'+
...
function callOCRandTranslate(imageBase64){
  ...
  var prompt='识别图片中英文并翻译。\n规则：先逐行输出图片中英文原文，再逐行输出中文翻译。\n不要分析语法，不要解释，不要加任何多余内容。\n\n格式：\n【英文原文】\n（第一行原文）\n（第二行原文）\n【中文翻译】\n（第一行译文）\n（第二行译文）';
    ...
    callAI(_scanConfig.apiUrl,_scanConfig.model,
      ...
      4096).then(function(text){
        ...
        for(var i=0;i<parts.length;i++){
          var p=parts[i];
          if(p.indexOf('英文原文】')===0){jp=p.replace('英文原文】','').trim()}
...
function toggleSubmenu(section) {
  if (_activeSubmenu === section) {
    closeSubmenu();
    return;
  }
  closeSubmenu();
  var el = document.getElementById('submenu' + section.charAt(0).toUpperCase() + section.slice(1));
  if (el) {
    el.classList.add('open');
    _activeSubmenu = section;
  }
}
function closeSubmenu() {
  document.querySelectorAll('.submenu-overlay').forEach(function(el) {
    el.classList.remove('open');
  });
  _activeSubmenu = null;
}
// Close submenu when tapping overlay outside
document.addEventListener('click', function(e) {
  if (_activeSubmenu) {
    var overlay = document.querySelector('.submenu-overlay.open');
    if (overlay && !overlay.contains(e.target) && !e.target.closest('.bottombar__item')) {
      closeSubmenu();
    }
  }
});
